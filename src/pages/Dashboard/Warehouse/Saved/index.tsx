...
import { ArrowDownArrowUpIcon } from '@/components/General';
...
function WarehouseSave() {
  ...
  useEffect(() => {
    ...
    const tb_header_local = JSON.parse(
      localStorage.getItem('warehouse_table-header_saved') ?? '{}',
    );
  ...
  const saveHeader = (data: ITableHeader) => {
    ...
    localStorage.setItem('warehouse_table-header_saved', JSON.stringify(cloneData));
  };
  return (
    <div className="mt-5">
      <div className="mt-3">
              ...
              <div className="w-[50%]" key={val[0]}>
                <Switch
                  ...
                  onChange={() => {
                    ...
                    if (
                      Object.entries(newData).filter((item) => (item[1].hidden ? false : true))
                        .length < 1
                    )
                      return window.alert('Phải hiện ít nhất 1 cột');
                    saveHeader(newData);
                    ...
                    {!tableHeader.spec?.hidden ? (
                      <td className="px-2">
                        <span className="line-clamp-1">
                          {data.spec.price} {data.spec.price}
                        </span>
                      </td>
                    ) : null}
