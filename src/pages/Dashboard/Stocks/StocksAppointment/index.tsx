...
export const StocksAppointment = () => {
  const ListDauKhachDatLich: TDauKhachDatLich[] = [
    // {
    //   id: '1',
    //   avatar:
    ...
    <div className="h-[calc(100vh_-_120px)] max-w-xl mx-auto bg-white lg:rounded-lg">
      <div className="px-5 py-3">
        <div className="flex items-center justify-between">
          <div className="text-sm font-semibold">Lịch sử Đầu khách đặt lịch xem nhà</div>
        </div>
        <div className="mt-3 border-b border-dividerLight/20"></div>
      </div>
      <div className="">
        <div className="flex flex-col gap-1 p-5">
          {ListDauKhachDatLich.length > 0 ? (
            ListDauKhachDatLich?.map((dauKhach) => (
              <div key={dauKhach.id}>
                <HouseViewHistory items={dauKhach} />
              </div>
            ))
          ) : (
